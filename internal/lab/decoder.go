...
func Decode(payload []byte, now time.Time) (model.LabResult, error) {
	...
	if err := json.Unmarshal(payload, &wire); err != nil {
		result := model.LabResult{ReceivedAt: now.UTC(), Revision: wire.Revision, COD: wire.COD, Ammonia: wire.Ammonia}
		result.SampleID, _ = uuid.Parse(wire.SampleID)
		result.BatchID, _ = uuid.Parse(wire.BatchID)
		result.LineID, _ = model.ParseLineID(wire.LineID)
		return result, err
	}
